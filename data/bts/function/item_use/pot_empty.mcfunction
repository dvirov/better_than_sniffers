...
#execute if block ~ ~ ~ decorated_pot summon item run function bts:item_use/summon_item_marker
...
execute if block ~ ~ ~ decorated_pot run execute as @e[limit=1,tag=blank_item ,distance=..0.01] at @s run data modify entity @s Item set from block ~ ~ ~ item
...
execute as @e[limit=1,tag=blank_item ,distance=..0.01] at @s run data remove block ~ ~ ~ item
