...
#execute  positioned ~ ~-1 ~ if entity @s[distance=..0.1,type=item_frame,tag=placed] run return run function bts:blocks/campfire/campfire_fail
#execute  positioned ~ ~-0.4 ~ if entity @s[distance=..0.1,type=item_frame,tag=placed] run return run function bts:blocks/campfire/campfire_fail
...
summon text_display ~ ~ ~ {Tags:["campfire","custom_block"],transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f],scale:[2f,2f,2f]},billboard:"center",text:'"text"',text:'{"score":{"name":"@s","objective":"campfire_time"}}'}
