...
execute if score @s campfire_time_ticks matches ..-0 run execute as @n[distance=..1,type=armor_stand,tag=campfire] at @s run function datapack:campfire/unlit
execute as @n[distance=..1,type=armor_stand,tag=campfire] at @s if block ~ ~ ~ minecraft:campfire[lit=false] run function datapack:campfire/unlit
