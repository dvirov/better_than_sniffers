execute as @s positioned ~ ~-1 ~ if entity @e[distance=..0.1,type=armor_stand,tag=placed] run return run function datapack:campfire/campfire_fail
execute as @s positioned ~ ~-0.4 ~ if entity @e[distance=..0.1,type=armor_stand,tag=placed] run return run function datapack:campfire/campfire_fail
